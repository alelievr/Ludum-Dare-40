...
Properties {
	...
	_DeformTexture ("Deform", 2D) = "white" {}
...
SubShader {
    ...
    Pass {
        ...
        struct v2f
        {
            ...
            half    skyGroundFactor : TEXCOORD4;
        ...
        v2f vert (appdata_t v)
        {
			...
			float4 olol = tex2Dlod(_DeformTexture, float4(eyeRay.xy + _Time.x / 10, 0, 0));

			eyeRay = normalize(olol * 1.5);
...
//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
			...
			float a = tex2Dlod(_DeformTexture, float4(cOut.xy, cOut.xy)).x *2;
		...
		float4 sin2hue(float c)
		{
			float4 C = .5+.5*cos(6.28*clamp(1.5*frac(c)-float4(0,.5,1,1.5),-.5,.5));
			return C+float4(C.w,0,0,0);
		}
        ...
        half4 frag (v2f IN) : SV_Target
        {
            ...
            half3 ray = normalize(mul((float3x3)unity_ObjectToWorld, IN.vertex.xyz));
